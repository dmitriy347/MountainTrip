...
def test_user_login_view_successful_login(client, user):
    ...
    assert response.url == reverse(
        "trip_list"
    )  # Перенаправление на страницу списка поездок (LOGIN_REDIRECT_URL)
...
def test_user_logout_view_successful_logout(auth_client):
    ...
    assert response.url == reverse(
        "home"
    )  # Перенаправление на главную страницу (LOGOUT_REDIRECT_URL)
